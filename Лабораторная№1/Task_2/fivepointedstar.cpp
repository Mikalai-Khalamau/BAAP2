...
const double k12 = 0.5;

Star5::Star5(double radius, QGraphicsItem* parent)
    : Figure(parent), m_radius_(radius), m_ratio_(k12) {}
...
double Star5::radius() const {
    return m_radius_;
}

void Star5::setRadius(double r) {
...
void Star5::paint(QPainter* painter, const QStyleOptionGraphicsItem* option,
                  QWidget* widget) {
    Q_UNUSED(option)
    Q_UNUSED(widget)
...
QPolygonF Star5::buildPolygon() const {
    QPolygonF poly;
    const int n = k5;  // 5 лучей &rarr; 10 вершин
...
double Star5::polygonArea(const QPolygonF& poly) {
    double a = 0.0;
    int n = poly.size();
...
double Star5::polygonPerimeter(const QPolygonF& poly) {
    double p = 0.0;
    int n = poly.size();
