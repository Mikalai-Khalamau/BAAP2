...
const int kTwo = 2.0;
const int kEight = 8.0;
const int k90 = 90;
const double k360 = 360.0;

Star8::Star8(double radius, QGraphicsItem* parent)
...
double Star8::radius() const {
    return m_radius_;
}

void Star8::setRadius(double r) {
...
void Star8::paint(QPainter* painter, const QStyleOptionGraphicsItem* option,
                  QWidget* widget) {
    Q_UNUSED(option)
    Q_UNUSED(widget)
...
QPolygonF Star8::buildPolygon() const {
    QPolygonF poly;
    const int n = kEight;  // 8 лучей &rarr; 16 вершин
...
double Star8::polygonArea(const QPolygonF& poly) {
    double a = 0.0;
    int n = poly.size();
...
double Star8::polygonPerimeter(const QPolygonF& poly) {
    double p = 0.0;
    int n = poly.size();
